namespace NetTools
{
    ...
    public static class BitsUtil
    {
        ...
        public static bool LtE(byte[] A, byte[] B, int offset = 0)
        {
            ...
            if (offset < 0)
            {
                throw new ArgumentException("offset must be greater than or equal 0.", "offset");
            }
            if ((A.Length <= offset) || (B.Length <= offset))
            {
                throw new ArgumentException("offset must be less than length of A and B.", "offset");
            }
        ...
        public static byte[] Decrement(byte[] bytes)
        {
            ...
            if (bytes.All(x => x == byte.MinValue))
            {
                throw new OverflowException();
            }
